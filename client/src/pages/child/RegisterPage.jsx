...
const RegisterPage = () => {
  ...
  const changeValue = (e) => {    
    ...
    if (e.target.name === "id" && e.target.value !== "") {
      axios({
        method: "GET",
        url: "http://localhost:9999/children/" + e.target.value,
        ...
        axios({
          ...
          .then((res) => {
            ...
            if (res.data.exists) {
              ...
              setParentExistMessage("부모님 찾기 완료! 회원가입 버튼을 눌러주세요");
              setIsParentExist(true);
            } else {
              setParentExistMessage("부모님이 존재하지 않습니다. 부모님께 확인해보세요");
      ...
      <StyledBack1>
        ...
        <StyledForm>
          ...
          <Form>
            ...
            <InputGroup className="mb-3">
              <InputGroup.Text style={isParentExist ? { color: "green" } : { color: "red" }}>
                {parentExistMessage}
              </InputGroup.Text>
            </InputGroup>
